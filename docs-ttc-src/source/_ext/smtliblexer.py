###############################################################################
# Sphinx extension: a Pygments lexer for SMT-LIBv2.
#
# Adapted from the cvc5 project (https://github.com/cvc5/cvc5), with the ttc
# specific commands `declare-projvar` and `declare-weight` added.
##
...
class SmtLibLexer(RegexLexer):
    """A lexer for SMT-LIBv2 that stays close to the SMT-LIBv2 standard while
    providing proper highlighting for everything ttc supports. The lexer
    implements the SMT-LIBv2 lexicon (section 3.1 of the standard) directly in
    the root state, as well as the commands, sorts and operators. Note that
    commands, sorts and operators are used to build regular expressions and,
    thus, can contain character classes (e.g. "[0-9]+"), but also need to
    ...
    COMMANDS = [
        ...
        # ttc counting extensions
        'declare-projvar', 'declare-weight',
        # SyGuS v2
